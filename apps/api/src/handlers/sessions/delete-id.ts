...
import { createErrorDataWithLogging, handleRepositoryError } from '../../utils/api-helpers';
...
export async function handleDeleteSessionById(c: Context<AppContext>) {
  ...
  try {
    ...
    if (!deleted) {
      const errorData = createErrorDataWithLogging(404, 'Session not found for deletion.');
