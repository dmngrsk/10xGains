...
|  | SESS-10 | A user can long-press the finish button, pick the date and time the session ended, and see the whole session recorded on that date. | High | No |
|  | SESS-11 | Finishing early from the finish options raises the same confirmation as the plain finish tap; cancelling leaves the session in progress. | High | No |
|  | SESS-12 | A user is prompted with a confirmation dialog if they try to complete a session with unfinished sets. | High | No |
| *&nbsp;&nbsp;notes* | SESS-13 | A user can open the notes dialog via the FAB in the session view, enter a session note, close the dialog with 'Save', and see the note again after reopening. | High | No |
|  | SESS-14 | The notes dialog is modal: a click outside neither closes it nor saves, and 'Cancel' discards the entered text. | High | No |
|  | SESS-15 | A plan note entered in one session is displayed when the notes dialog is opened in another session of the same plan. | High | No |
|  | SESS-16 | A plan note from one plan is never displayed in a session belonging to a different plan. | Critical | No |
|  | SESS-17 | A user cannot read or modify another user's notes (RLS check). | Critical | No |
| *&nbsp;&nbsp;history integrity* | SESS-18 | Editing a set in one session leaves the recorded sets of earlier completed sessions of the same plan day byte-for-byte unchanged. | Critical | No |
