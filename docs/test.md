...
|  | SESS-08 | A user can long-press the finish button, pick the date and time the session ended, and see the whole session recorded on that date. | High | No |
|  | SESS-09 | Finishing early from the finish options raises the same confirmation as the plain finish tap; cancelling leaves the session in progress. | High | No |
