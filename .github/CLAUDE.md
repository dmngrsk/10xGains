...
- Both apps are built in CI (`build-api` and `build-web` run in parallel) and passed to CD as `dist-api-<environment>` / `dist-web-<environment>` artifacts; CD jobs only download and deploy. This relies on CI and CD running in the same workflow run.
