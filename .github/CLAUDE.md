...
- The Azure Functions deploy uses OIDC (`azure/login@v2` + `Azure/functions-action@v1` with `remote-build: true`, `sku: flexconsumption`). Jobs using OIDC need `permissions: id-token: write`, and reusable-workflow callers must grant it too.
