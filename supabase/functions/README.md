...
- `supabase/functions/shared/` - Contains shared utilities and types used across multiple functions.
  - `models/api-types.ts` - Shared API DTOs and Command model type definitions.
  - `models/database-types.ts` - Database type definitions (auto-generated by Supabase).
  - `services/[business-logic].ts` - Business logic related to executing some of the functions. 
  - `utils/api-handler.ts` - Core utilities for the advanced routing mechanism (e.g., `createMainRouterHandler`, `routeRequestToMethods`, `ApiHandlerContext`).
  - `utils/api-helpers.ts` - Helper functions for creating consistent API responses (e.g., `createSuccessResponse`, `createErrorResponse`), CORS headers, and other common utilities.
...
2.  The `copy-api-types.js` script copies these to:
    *   `supabase/functions/shared/models/api-types.ts`
    *   `supabase/functions/shared/models/database-types.ts`
...
The User Profiles API is served by the `supabase/functions/user-profiles` Edge Function. It allows authenticated users to manage their own profile information.
...
-   **URL Path Parameter**:
    -   `id` (UUID): The ID of the user profile to retrieve.
