...
export async function handleGetPlanExerciseSetById(c: Context<AppContext>) {
  ...
  try {
    ...
    if (!set) {
      const errorData = createErrorDataWithLogging(404, 'Plan exercise set not found or user does not have access.');
      return c.json(errorData, 404);
    }

    const successData = createSuccessData<PlanExerciseSetDto>(set);
    return c.json(successData, 200);
  } catch (error) {
    const fallbackMessage = 'Failed to get plan exercise set';
    return handleRepositoryError(c, error as Error, planRepository.handlePlanOwnershipError, handleGetPlanExerciseSetById.name, fallbackMessage);
  }
}
