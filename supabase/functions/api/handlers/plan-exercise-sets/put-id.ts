...
import type { PlanExerciseSetDto, UpdatePlanExerciseSetCommand } from '../../models/api.types.ts';
...
export async function handlePutPlanExerciseSetById(c: Context<AppContext>) {
  const { path, error: pathError } = validatePathParams(c, PATH_SCHEMA);
  if (pathError) return pathError;

  const { command, error: commandError } = await validateCommandBody<typeof COMMAND_SCHEMA, UpdatePlanExerciseSetCommand>(c, COMMAND_SCHEMA);
  ...
  try {
    ...
    if (!updatedSet) {
      const errorData = createErrorDataWithLogging(404, 'Plan exercise set not found for update.');
      return c.json(errorData, 404);
    }

    const successData = createSuccessData<PlanExerciseSetDto>(updatedSet);
    return c.json(successData, 200);
  } catch (error) {
    const fallbackMessage = 'Failed to update plan exercise set';
    return handleRepositoryError(c, error as Error, planRepository.handlePlanOwnershipError, handlePutPlanExerciseSetById.name, fallbackMessage);
  }
}
