...
import type { PlanExerciseSetDto, CreatePlanExerciseSetCommand } from '../../models/api.types.ts';
...
export async function handleCreatePlanExerciseSet(c: Context<AppContext>) {
  const { path, error: pathError } = validatePathParams(c, PATH_SCHEMA);
  if (pathError) return pathError;

  const { command, error: commandError } = await validateCommandBody<typeof COMMAND_SCHEMA, CreatePlanExerciseSetCommand>(c, COMMAND_SCHEMA);
  ...
  try {
    ...
    const successData = createSuccessData<PlanExerciseSetDto>(newSet);
