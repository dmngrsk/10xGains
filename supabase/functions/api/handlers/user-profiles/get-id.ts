...
export async function handleGetUserProfile(c: Context<AppContext>) {
  ...
  try {
    ...
    const successData = createSuccessData<UserProfileDto>(profile);
    return c.json(successData, 200);
  } catch (e) {
    const fallbackMessage = 'Failed to get user profile';
    return handleRepositoryError(c, e as Error, profileRepository.handleProfileError, handleGetUserProfile.name, fallbackMessage);
  }
}
