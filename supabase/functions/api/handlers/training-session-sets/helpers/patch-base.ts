...
export async function patch(
  ...
  try {
    ...
    if (!currentSet) {
      const errorData = createErrorDataWithLogging(404, 'Training session set not found.');
    ...
    if (!updatedSet) {
      const errorData = createErrorDataWithLogging(404, 'Training session set not found for update.');
      return c.json(errorData, 404);
    }

    const successData = createSuccessData(updatedSet, { message: 'Training session set updated successfully.' });
    return c.json(successData, 200);
  } catch (e) {
    if ((e as Error).message.includes('Training session') && (e as Error).message.includes('is completed')) {
