...
import type { UpdatePlanExerciseCommand } from '../../models/api.types.ts';
...
export async function handlePutPlanExerciseById(c: Context<AppContext>) {
  const { path, error: pathError } = validatePathParams(c, PATH_SCHEMA);
  if (pathError) return pathError;

  const { command, error: commandError } = await validateCommandBody<typeof COMMAND_SCHEMA, UpdatePlanExerciseCommand>(c, COMMAND_SCHEMA);
  ...
  try {
    ...
    if (!updatedExercise) {
      const errorData = createErrorDataWithLogging(404, 'Plan exercise not found for update.');
    ...
    const fallbackMessage = 'Failed to update plan exercise';
    return handleRepositoryError(c, error as Error, planRepository.handlePlanOwnershipError, handlePutPlanExerciseById.name, fallbackMessage);
  }
}
