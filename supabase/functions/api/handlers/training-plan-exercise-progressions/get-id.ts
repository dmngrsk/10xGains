...
export async function handleGetTrainingPlanExerciseProgressionById(c: Context<AppContext>) {
  ...
  try {
    ...
    if (!progression) {
      const errorData = createErrorDataWithLogging(404, 'Training plan exercise progression not found or user does not have access.');
    ...
    const fallbackMessage = 'Failed to get training plan exercise progression';
    return handleRepositoryError(c, error as Error, planRepository.handlePlanOwnershipError, handleGetTrainingPlanExerciseProgressionById.name, fallbackMessage);
  }
}
