...
import type { PlanExerciseProgressionDto, UpsertPlanExerciseProgressionCommand } from '../../models/api.types.ts';
...
export async function handlePutPlanExerciseProgressionById(c: Context<AppContext>) {
  const { path, error: pathError } = validatePathParams(c, PATH_SCHEMA);
  if (pathError) return pathError;

  const { command, error: commandError } = await validateCommandBody<typeof COMMAND_SCHEMA, UpsertPlanExerciseProgressionCommand>(c, COMMAND_SCHEMA);
  ...
  try {
    ...
    const successData = createSuccessData<PlanExerciseProgressionDto>(upsertedProgression);
