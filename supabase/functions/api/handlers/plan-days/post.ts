...
import type { CreatePlanDayCommand, PlanDayDto } from '../../models/api.types.ts';
...
export async function handleCreatePlanDay(c: Context<AppContext>) {
  const { path, error: pathError } = validatePathParams(c, PATH_SCHEMA);
  if (pathError) return pathError;

  const { command, error: commandError } = await validateCommandBody<typeof COMMAND_SCHEMA, CreatePlanDayCommand>(c, COMMAND_SCHEMA);
  ...
  try {
    ...
    const successData = createSuccessData<PlanDayDto>(newDay);
