...
export async function handleGetTrainingPlanDays(c: Context<AppContext>) {
  ...
  try {
    ...
    const fallbackMessage = 'Failed to get training plan days';
    return handleRepositoryError(c, error as Error, planRepository.handlePlanOwnershipError, handleGetTrainingPlanDays.name, fallbackMessage);
  }
}
