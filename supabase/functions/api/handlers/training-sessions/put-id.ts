...
export async function handlePutTrainingSessionById(c: Context<AppContext>) {
  ...
  try {
    ...
    if (!updatedSession) {
      const errorData = createErrorDataWithLogging(404, 'Training session not found for update.');
    ...
    return handleRepositoryError(c, e as Error, sessionRepository.handleSessionOwnershipError, handlePutTrainingSessionById.name, fallbackMessage);
  }
}
