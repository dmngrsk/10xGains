...
- `packages/shared` - Shared code (`@txg/shared`): API DTOs, command models, and generated database types. Types only, with one exception: `src/domain.types.ts` also exports runtime lists of the value unions the database constrains columns to (statuses, deload strategies, measurement types), because validators need the values and not just the type. Derive from the generated types there rather than retyping a union by hand.
